...
class Config(object):
    ...
    def __init__(self):
        ...
        self.results_dir = os.path.join(self.base_path, 'data/results')
        self.db_path = os.path.join(self.base_path, 'data/results.db')
...
class Controller:
    ...
    def _write_result_table(self, table_name):
            
            col_names = ['instanceName', 'numSource', 'numTarget', 'pNorm', 'fairnessCoefficient', 'AssignmentCost', 'TotalCost', 'GapToOpt', 'computationTimeInSec'] 
            create_table(self._cursor, table_name, col_names)

            dir_path = self.config.results_dir + '/' + table_name
            for f in os.listdir(dir_path):
                ...
                with open(os.path.join(dir_path, f),'r') as fin:
                    ...
                    table_values = [result_dict['instanceName'], result_dict['numSource'], result_dict['numTarget'], result_dict['pNorm'], result_dict['fairnessCoefficient'], result_dict['assignmentCosts'], round(result_dict['objectiveValue'], 4), round(result_dict['optimalityGapPercent']/100, 2), result_dict['computationTimeInSec']]
